...
class Ticket(BaseModel):
    ...
    priority: Literal["низкий", "средний", "высокий"] = Field(
        description=(
            ...
            "низкий — обычный рабочий вопрос. "
            "Оценивай по фактам обращения, а не по эмоциям и восклицательным знакам."
        )
    )
    ...
    def check_equipment_id(cls, value: Optional[str]) -> Optional[str]:
        ...
        Правила:
          ...
          - Если значение есть — нормализуем написание и ищем в реестре;
            возвращаем каноничную запись («км 101» -> «КМ-101»).
          - В реестре нет (в т.ч. выдуманный моделью код) — ValueError,
            заявка уходит на ручную проверку, а не в базу.
