"""
Модуль 2, Задание 6 — структурированный ответ и защита от некорректного ввода.

В Задании 5 модель отвечала словом, и человек его читал. Здесь ответ модели
...
# Идентификатор оборудования на ПромТехе: две буквы, дефис, номер. КМ-101, НМ-205, П-7 — нет.
EQUIPMENT_PATTERN = re.compile(r"^[А-ЯA-Z]{1,3}-\d{1,4}$")
...
class Ticket(BaseModel):
    ...
    equipment_id: Optional[str] = Field(
        default=None,
        description="TODO 1: идентификатор оборудования; что ставить, если он не назван",
    )
    ...
    def check_equipment_id(cls, value: Optional[str]) -> Optional[str]:
        """TODO 2: проверить формат идентификатора оборудования.

        Правила:
          - None — это нормально: оборудование могли не назвать.
          - Если значение есть, оно должно совпадать с EQUIPMENT_PATTERN (КМ-101).
          - Если не совпадает — raise ValueError с понятным текстом.
            Такая заявка не пройдёт схему и уйдёт на ручную проверку. Это правильно:
            лучше показать человеку, чем записать в базу «КМ 101» и потерять его.
