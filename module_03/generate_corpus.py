"""
Модуль 3, Занятие 5 — генерация внутренних документов ПромТеха через GigaChat.
...
Важно, почему проверять RAG будем именно по сгенерированным документам:
публичные ФНП и ГОСТы могли попасть в обучающую выборку модели — тогда
непонятно, отвечает она ПО БАЗЕ или ПО ПАМЯТИ. Сгенерированный вчера
регламент в обучении быть не мог: если модель называет срок из него —
значит, RAG действительно работает.
...
Запуск (из папки module_03):
    python generate_corpus.py                 # всё подряд
...
Результат:
    data/generated/  — сырые JSON/markdown от модели
    data/corpus/gen_*.md — документы корпуса с метаданными
"""
...
REGISTRY = json.loads(
    (BASE_DIR / "data" / "equipment_registry.json").read_text(encoding="utf-8")
)["equipment"]
...
def validate_equipment_id(value) -> str | None:
    ...
    Что сделать (всё уже готово выше — как в Задании 6 модуля М2):
...
def write_corpus_file(name: str, doc_id: str, doc_type: str, title: str,
                      equipment: list, date: str, text: str):
    eq = "[" + ", ".join(equipment) + "]" if equipment else "[]"
...
def clear_old(prefix: str):
    for f in CORPUS_DIR.glob(f"{prefix}*.md"):
        f.unlink()
...
def generate_cards() -> list:
    ...
    # каждая карточка — отдельный документ корпуса
    clear_old("gen_card_")
    ...
    for i, card in enumerate(cards, 1):
        ...
        text = (
            f"# Карточка оборудования {eq_id} — {names[eq_id]}\n\n"
...
def generate_reglaments():
    ...
    for i, topic in enumerate(REGLAMENT_TOPICS, 1):
        ...
        print(f"    OK: gen_regl_{i:02d}.md — {len(text)} символов")
