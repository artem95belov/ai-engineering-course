...
def chunk_id(record: dict, chunk_no: int) -> str:
    ...
    Считается из того, чем чанк ЯВЛЯЕТСЯ (документ, страница или раздел,
    номер куска), а не из того, каким по счёту его обработали. Иначе после
    правки одного документа сдвинутся id у всех следующих, и обновить
    в базе один документ станет невозможно — только пересобрать всё.
    """
    key = f"{record['doc_id']}|{record.get('page')}|{record.get('section')}|{chunk_no}"
